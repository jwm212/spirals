...
apptainer run --bind ~/projects/nhm/jmcdermo/:/mnt --pwd /mnt/spirals/kailidiscus/velocity/${cases[$SLURM_ARRAY_TASK_ID]} ~/apps/openfoam_12.sif bash -c "
decomposePar &&
...
time cp ~/projects/nhm/jmcdermo/spirals/kailidiscus/velocity/${cases[$SLURM_ARRAY_TASK_ID]}/VTK/*.vtk .
echo "compressing VTK files..."
time tar -cvf VTKs.tar *.vtk
time pigz -p 24 VTKs.tar

mv VTKs.tar.gz ~/projects/nhm/jmcdermo/spirals/kailidiscus/velocity/${cases[$SLURM_ARRAY_TASK_ID]}/VTK
echo "done"
