...
# -*- coding: utf-8 -*-
# Run using `pvbatch U_profiles_extract.py`, needs ~ 16GB RAM.
...
for j in arr1:
    ...
    for i in arr2:
        ...
        plotOverLine1.Point1 = [2*L[i], 0.0, 0.0]
        plotOverLine1.Point2 = [2*L[i], 0.0, 1.25*L[i]]
        UpdatePipeline(time=0.0, proxy=plotOverLine1)
        SaveData('../'+taxon[j]+'/velocity/'+cases[i]+'/postProcessing/'+'2L_t_avrgd.csv', proxy=plotOverLine1, PointDataArrays=['U', 'arc_length'])

        plotOverLine2 = PlotOverLine(registrationName='PlotOverLine2', Input=temporalStatistics1)
        plotOverLine2.Point1 = [3*L[i], 0.0, 0.0]
        plotOverLine2.Point2 = [3*L[i], 0.0, 1.25*L[i]]
        UpdatePipeline(time=0.0, proxy=plotOverLine2)
        SaveData('../'+taxon[j]+'/velocity/'+cases[i]+'/postProcessing/'+'3L_t_avrgd.csv', proxy=plotOverLine2, PointDataArrays=['U', 'arc_length'])

        plotOverLine3 = PlotOverLine(registrationName='PlotOverLine3', Input=temporalStatistics1)
        plotOverLine3.Point1 = [4*L[i], 0.0, 0.0]
        plotOverLine3.Point2 = [4*L[i], 0.0, 1.25*L[i]]
        UpdatePipeline(time=0.0, proxy=plotOverLine3)
        SaveData('../'+taxon[j]+'/velocity/'+cases[i]+'/postProcessing/'+'4L_t_avrgd.csv', proxy=plotOverLine3, PointDataArrays=['U', 'arc_length'])
